...
export default function ProviderDetailPage() {
  ...
  const handleSaveToggle = () => {
    ...
    const currentlySaved = isProviderSaved(provider.id);
    if (currentlySaved) {
  ...
  const providerIsSaved = user ? isProviderSaved(provider.id) : false;
          ...
          <div className="relative h-64 md:h-96 w-full">
            ...
            <div className="absolute bottom-0 left-0 p-6 md:p-8">
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                {provider.isVerified && <BadgeCheck className="h-7 w-7 text-white fill-primary" />}
              </div>
               <div className="mt-2 flex items-center gap-2 text-yellow-400">
                    <Star className="h-5 w-5 fill-current" />
                    <span className="font-bold text-lg text-white">{provider.rating.toFixed(1)}</span>
                    <span className="text-sm text-white/80">({provider.reviewCount} reviews)</span>
                </div>
            </div>
          </div>

            ...
            <div className="absolute bottom-0 left-0 p-6 md:p-8">
              ...
              <h1 className="font-headline text-3xl md:text-5xl font-bold text-white mt-2">
               ...
               <div className="mt-2 flex items-center gap-2 text-yellow-400">
                ...
                <span className="font-bold text-white">{details.reviews.rating}</span>
                <span className="text-sm text-gray-300">({details.reviews.count} reviews)</span>
              </div>
            </div>
          </div>
                  ...
                  <CardContent className="p-4 space-y-4">
                    <h4 className="font-semibold font-headline mb-2">Contact Information</h4>
                     <div className="flex items-start gap-4">
                      ...
                      <div>
                         <p className="text-muted-foreground text-sm">{details.contact.address}</p>
                      </div>
                    </div>
                     <div className="flex items-start gap-4">
                      <Phone className="h-5 w-5 mt-1 text-primary flex-shrink-0" />
                      <div>
                         <p className="text-muted-foreground text-sm">{details.contact.phone}</p>
                      </div>
                    </div>
                     <div className="flex items-start gap-4">
                      <Mail className="h-5 w-5 mt-1 text-primary flex-shrink-0" />
                      <div>
                         <p className="text-muted-foreground text-sm hover:text-primary"><a href={`mailto:${details.contact.email}`}>{details.contact.email}</a></p>
                      </div>
                    </div>
                     <div className="flex items-start gap-4">
                      <Globe className="h-5 w-5 mt-1 text-primary flex-shrink-0" />
                      <div>
                         <p className="text-muted-foreground text-sm hover:text-primary"><a href={`https://${details.contact.website}`} target="_blank" rel="noopener noreferrer">{details.contact.website}</a></p>
                      </div>
                    </div>
                  </CardContent>
                </Card>

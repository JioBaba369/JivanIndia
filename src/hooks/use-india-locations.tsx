...
const indiaLocationData: LocationData = {
...
interface IndiaLocationsContextType {
  states: string[];
  getDistrictsByState: (state: string) => string[];
}

const IndiaLocationsContext = createContext<IndiaLocationsContextType | undefined>(undefined);

export function IndiaLocationsProvider({ children }: { children: ReactNode }) {
  const [states] = useState<string[]>(() => Object.keys(indiaLocationData).sort());

  const getDistrictsByState = useCallback((state: string): string[] => {
    return indiaLocationData[state]?.sort() || [];
  }, []);

  const value = { states, getDistrictsByState };

  return (
    <IndiaLocationsContext.Provider value={value}>
      {children}
    </IndiaLocationsContext.Provider>
  );
}

export function useIndiaLocations() {
  const context = useContext(IndiaLocationsContext);
  if (context === undefined) {
    throw new Error('useIndiaLocations must be used within a IndiaLocationsProvider');
  }
  return context;
}
